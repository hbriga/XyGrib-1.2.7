...
//        ind = 0;
    ...
    //
    else if (amod == "INMET 0.025°"   || amod == "INMET 0.0625°"  )			//HB
    {																		//HB
        // set res, days, cyc options										//HB
																			//HB
        cbResolution->clear();												//HB
        if (amod == "INMET 0.025°") {										//HB
	        cbResolution->addItems(QStringList()<< "0.025");				//HB
	    }  																	//HB
        if (amod == "INMET 0.0625°") {										//HB
	    	cbResolution->addItems(QStringList()<< "0.0625");				//HB
	    }	 																//HB
        cbResolution->setEnabled(false);									//HB
        cbDays->clear();													//HB
        if (amod == "INMET 0.025°") {										//HB
        	cbDays->addItems(QStringList()<< "1"<<"2");						//HB
        }																	//HB
        if (amod == "INMET 0.0625°") {										//HB
        	cbDays->addItems(QStringList()<< "1"<<"2"<<"3"<<"4"<<"5"<<"6");	//HB
        }																	//HB	                
        ind = Util::getSetting("downloadIndNbDays", 2).toInt();				//HB
        ind = Util::inRange(ind, 0, cbDays->count()-1);						//HB
        cbDays->setCurrentIndex(ind);										//HB
        ind = 0;															//HB
        cbInterval->clear();												//HB
//      cbInterval->addItems(QStringList()<<"1"<<"3"<<"6"<<"12");			//HB
        cbInterval->addItems(QStringList()<<"1");							//HB   
        cbInterval->setEnabled(false);				  						//HB             
//      cbInterval->setMinimumWidth (0);									//HB
//      ind = Util::getSetting("downloadIndInterval", 0).toInt();			//HB
//      if (ind == 24) ind = 1;												//HB
//      ind = Util::inRange(ind, 0, cbInterval->count()-1);					//HB
//      cbInterval->setCurrentIndex(ind);									//HB
        ind = 0;															//HB
        cbRunCycle->clear();												//HB
        cbRunCycle->insertItem (ind++, tr("Last"), "last");					//HB
        cbRunCycle->insertItem (ind++, tr("0 hr"), "00");					//HB
        cbRunCycle->insertItem (ind++, tr("12 hr"), "12");					//HB

        // reactivate parameters that may have been disabled				//HB
        chkWind->setEnabled(true);											//HB
        chkGUSTsfc->setEnabled(true);										//HB
        chkPressure->setEnabled(true);										//HB
        chkTemp->setEnabled(true);											//HB
        chkHumid->setEnabled(true);											//HB
        chkCAPEsfc->setEnabled(true);										//HB
        chkCINsfc->setEnabled(true);										//HB
        chkReflectivity->setEnabled(true);									//HB 
        if (amod == "INMET 0.0625°") {										//HB	Doesn't have reflectivity on this model
        	 chkReflectivity->setEnabled(false);							//HB
        	 chkReflectivity->setChecked(false);							//HB        	 
        }    																//HB     
        chkCloud->setEnabled(true);											//HB                       
        chkRain->setEnabled(true);											//HB
            
        // deactivate unvalid parameters										//HB
        chkIsotherm0->setEnabled(false); chkIsotherm0->setChecked(false);		//HB        
        chkSnowCateg->setEnabled(false); chkSnowCateg->setChecked(false);		//HB
        chkFrzRainCateg->setEnabled(false); chkFrzRainCateg->setChecked(false); //HB
        chkSnowDepth->setEnabled(false); chkSnowDepth->setChecked(false);		//HB

        chkAltitude200->setEnabled(true);										//HB
        chkAltitude300->setEnabled(true);										//HB
        chkAltitude400->setEnabled(true);										//HB
        chkAltitude500->setEnabled(true);										//HB
        chkAltitude600->setEnabled(true);										//HB
        chkAltitude700->setEnabled(true);										//HB
        chkAltitude850->setEnabled(true);										//HB
        chkAltitude925->setEnabled(true);										//HB

        chkAltitude_All->setEnabled(true);										//HB
        chkAltitude_SkewT->setEnabled(true);									//HB

        // deactivate unvalid parameters										//HB

        chkAltitude200->setEnabled(false); chkAltitude200->setChecked(false);	//HB
        chkAltitude300->setEnabled(false); chkAltitude300->setChecked(false);	//HB
        chkAltitude400->setEnabled(false); chkAltitude400->setChecked(false);	//HB
        chkAltitude500->setEnabled(false); chkAltitude500->setChecked(false);	//HB
        chkAltitude600->setEnabled(false); chkAltitude600->setChecked(false);	//HB
        chkAltitude700->setEnabled(false); chkAltitude700->setChecked(false);	//HB
        chkAltitude850->setEnabled(false); chkAltitude850->setChecked(false);	//HB
        chkAltitude925->setEnabled(false); chkAltitude925->setChecked(false);	//HB

        chkAltitude_All->setEnabled(false); chkAltitude_All->setChecked(false);		//HB
        chkAltitude_SkewT->setEnabled(false); chkAltitude_SkewT->setChecked(false);	//HB
    }																				//HB
    //
...
//    sbEast->setSuffix(tr(" °E"));
    ...
    cbModel->addItems(QStringList()<< tr("None") << "GFS"<< "ICON" << "Arpege" << "ECMWF"
                                    ...
                                    << "Arpege-EU" << "Arome 0.025°" << "INMET 0.025°" << "INMET 0.0625°"  );  	//HB
//                                  << "Arpege-EU" << "Arome 0.025°" );								   			//HB                                                                      
