...
//    if (cloudLayers) parameters += "L;";
    ...
    if (atmModel == "GFS" && resolution == 0.25 ){
        ...
        amod = "ecmwf_p50_";
    } else if (atmModel == "INMET 0.025°"){							//HB
        amod = "inmet_2-8km_";        								//HB
    } else if (atmModel == "INMET 0.0625°"){						//HB
        amod = "inmet_7km_";        								//HB                
...
//
//	    ******************************************************************************* 			//HB
//		*** IMMET Gribs are not located at opengribs server = grbsrv.opengribs.org  ***				//HB
//		***    so we need to pass server name or ip using -inmetsrv arg, like that: ***				//HB
//      ***          XyGrib -inmetsrv:192.168.15.23                                 ***				//HB
//      ***  will be safe to remove these lines when opengribs 						***				//HB
//      ***  starts serving INMET files!											***     		//HB
//	    ******************************************************************************* 			//HB
		QString serverName = "";																	//HB
        if  (amod == "inmet_2-8km_"    ||    amod == "inmet_7km_") {                            	//HB
			serverName  = Util::getSetting("inmetServerName", "").toString();						//HB
			page = "";																				//HB
			QTextStream(&page) << "/api/v1/resources/xygrib/area?"									//HB
			                   << "osys=" << ptype													//HB
                               << "&ver=" << appVer													//HB
                               << "&model=" << amod													//HB
                               << "&latNE=" << y0													//HB
			...
			serverName  = Util::getServerName();													//HB
		} 																							//HB
																									//HB
//      QNetworkRequest request = Util::makeNetworkRequest ("http://"+Util::getServerName()+page);	//HB					
        QNetworkRequest request = Util::makeNetworkRequest ("http://"+serverName+page);				//HB		
